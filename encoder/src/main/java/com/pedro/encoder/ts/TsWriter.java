 ...
 * --------------------------------- Overview of PES/TS------------ -----------------------------
 *
 * PES (a PES is cut into N segments, and each segment composes the load of a TS packet)
 * TS 1
 * TS 2
 * TS ...
 * TS N
 * The first TS packet = TS header + TS adaptive field (8bit) + PES header + PES optional header + NALU AUD + SEI + SPS + PPS + TS payload (IDR Frame)
 * The second TS packet to the penultimate TS packet of this frame:
 * Fixed format: TS = TS_header(4bytes) + TS_payload (184bytes)
 * The only change is the field ContinuityCounter (packet increment counter) in Ts_header, which changes cyclically from 0-15
 * The last TS packet = TS header + TS adaptation field + padding field + TS Payload
 *
...
public class TsWriter {
	...
	public byte[] writeH264(boolean isFirstPes, byte[] h264Buf, int length, long pts, long dts) {
		...
		FrameData frameData = new FrameData();
		frameData.buf = h264Buf;
		frameData.pts = pts;
		frameData.dts = dts;
		frameData.isAudio = false;
		
		return write(isFirstPes, FrameDataType.MIXED, frameData);
	}
			...
			while ( frameBufPtr < frameBufSize ) {
				...
				if ( isFristTs ) {
					...
					if ( PTS_DTS_flags == (byte)0xc0 ) {
		     		...
		     		// H264 NAL
					if ( !isAudio && bytesIndexOf(frameBuf, H264_NAL ) == -1 ) {
	...
	int bytesIndexOf(byte[] array, byte[] target) {
	...
	public static class FrameData {
		...
		public byte[] buf; // A reference to the reference-counted buffer where the packet data is stored
		public long pts; // Presentation timestamp in time_base units; the time at which the decompressed packet will be presented to the user.
		public long dts; // Decompression timestamp in time_base units; the time at which the packet is decompressed
	}
