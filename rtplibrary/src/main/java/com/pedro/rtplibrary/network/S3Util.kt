...
class S3Util {
  ...
  fun getS3Client(context: Context): AmazonS3Client {
    if (sS3Client == null) {
      val credentials = getCredProvider(context)
      sS3Client =
        AmazonS3Client(credentials, Region.getRegion(US_WEST_2))
      try {
   ...
   * @throws IOException
   */
  @Throws(IOException::class) fun copyContentUriToFile(
    context: Context,
    uri: Uri?
  ): File {
    val `is` = context.contentResolver
        .openInputStream(uri!!)
    val copiedData = File(
  ...
  fun fillMap(
    ...
    val progress = (observer.bytesTransferred
        .toDouble() * 100 / observer
        .bytesTotal) as Int
    map["id"] = observer.id
  ...
  companion object {
    val TAG = S3Util::class.java.simpleName
  }
}